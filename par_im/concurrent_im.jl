...
function make_serial_block(N::Int, par::Params)
    nblocks = 1
    n_extra = 0
    size_per_block = N
    if par.size < N
        nblocks = ceil(N / par.size)
        n_extra = N % par.size
        size_per_block = Int(floor(N / nblocks))
    end
    internal_indices = []
    low = 1
    for i in 1:nblocks
        ...
        indexrows = low:low+bump
        push!(internal_indices, indexrows)
        low += (bump + 1)
    end
